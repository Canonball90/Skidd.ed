...
import skidd.ed.clickgui.ClickGui;
...
public class ModuleInitializer {
    ...
    public void addModules(String folder) {
        try {
            final List<Class<?>> classes = ClassFinder.from("skidd.ed.modules." + folder);
            if (classes == null)
                return;
    ...
    public void onKey(int eventKey) {
        if (eventKey == 0 || !Keyboard.getEventKeyState() || Skidded.mc.currentScreen instanceof ClickGui)
            return;
