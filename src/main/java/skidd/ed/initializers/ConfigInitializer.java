...
public class ConfigInitializer {
    ...
    public void init() {
        path = new File(Skidded.mc.gameDir + File.separator + "skidded");
        if (!path.exists())
            path.mkdir();
        modules.addAll(Skidded.moduleInitializer.getModuleList());
        load();
    }
    ...
    public void saveFriendList() {
        try {
            File file = new File(path + File.separator + "Friends.skid");
            if (!file.exists())
                file.createNewFile();
            BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(file));
            for (FriendInitializer.FriendPlayer friendPlayer : Skidded.friendInitializer.getFriendList()) {
    ...
    public void loadFriendList() {
        try {
            ...
            bufferReader.lines().forEach(line -> {
                String name = line;
                Skidded.friendInitializer.addFriend(name);
            });
        } catch (Exception ignored) {
        }
    }
        ...
        try {
            for (Module module : modules) {
                ...
                if (!categoryPath.exists())
                    categoryPath.mkdir();
                File file = new File(categoryPath.getAbsolutePath(), module.getName() + ".skid");
                if (!file.exists())
                    file.createNewFile();
                BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(file));
    ...
    public void saveHudState() {
        try {
            for (HudModule hudModule : Skidded.hudComponentInitializer.getHudModules()) {
                File categoryPath = new File(path.getAbsolutePath() + File.separator + "hud");
                if (!categoryPath.exists())
                    categoryPath.mkdir();
                File file = new File(categoryPath.getAbsolutePath(), hudModule.getName() + ".skid");
                if (!file.exists())
                    file.createNewFile();
                BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(file));
